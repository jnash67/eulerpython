...
for mindiffi in range(3,maxn):
    ...
    maxindexrangeofdiff = int(math.sqrt(2/3*diff)+1)
    if (mindiffi+maxindexrangeofdiff > highestPInHigherList):
        for i in range(highestPInHigherList,mindiffi+maxindexrangeofdiff):
            higher.append(pentagonals[i])
        highestPInHigherList = mindiffi+maxindexrangeofdiff
